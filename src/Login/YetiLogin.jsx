...
const YetiLogin = () => {
  ...
  useEffect(() => {
    ...
    // Replace the existing stopBlinking function with this safe guarded version
  const stopBlinking = () => {
    if (blinking) {
      // kill the GSAP tween/timeline only if it exists
      blinking.kill();
      blinking = null;
    }
    ...
    return () => {
      ...
      // FIXED: removeEventListener should receive the same handler reference that was added
  ...
  const handleSubmit = async (e) => {
    ...
    // simple validation so we actually use setErrorMessage/setShowError and email/password (avoids unused-var warnings)
    if (!email || !password) {
